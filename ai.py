...
class Agent:
    ...
    def simulate_trajectory(self):
        ...
        while True:
            ...
            rewards.append(self.simulator.check_reward())
            trajectory.append((state, action))
            state =  self.make_one_transition(action)
            if self.simulator.game_over() == True:
                rewards.append(self.simulator.check_reward())
                trajectory.append((self.simulator.state, action))
                break
        return trajectory, rewards
    
    def reward_to_go(self, rewards, index):
        reward_to_go = 0;
        for i in range(index,len(rewards), +1):
            #print("reward to go for loop index is", i)
            reward_to_go += rewards[i] * DISCOUNT ** (i - index)
        return reward_to_go
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            trajectory, rewards = self.simulate_trajectory()
            #print(rewards)
            for i in range(0, len(trajectory), +1):
                 rewardToGo =self.reward_to_go(rewards, i)
                 state, _ = trajectory[i]
                 self.S_MC[state] += rewardToGo
                 self.N_MC[state] += 1
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while True:
                ...
                #print("The state is", state, "The reward is", reward)
                next_state = self.make_one_transition(action)
                self.N_TD[state] += 1
                self.TD_values[state] = self.TD_values[state] + self.alpha(self.N_TD[state]) * (reward + DISCOUNT * self.TD_values[next_state] - self.TD_values[state])
                state = next_state
                if (self.simulator.game_over()):
                    self.TD_values[state] = self.simulator.check_reward()
                    self.N_TD[state] += 1
                    break
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while True:
                ...
                self.Q_values[state][action] = self.Q_values[state][action] + self.alpha(self.N_Q[state][action]) * (reward + DISCOUNT * max(self.Q_values[next_state][HIT]- self.Q_values[state][action], self.Q_values[next_state][STAND]- self.Q_values[state][action]))
                state = next_state
                if (self.simulator.game_over()):
